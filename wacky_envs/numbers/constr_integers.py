...
class IntConstr(WackyInt):
    ...
    def _init_val(val) -> [None, WackyInt, WackyMath]:
        '''Checks if parameter values are the right type. Converts integers to WackyInt.'''
        if val is None:
            return None
    ...
    def step(self, t: float,  delta_t: float, episode_delta_t: float) -> None:
        ...
        if self.func_time is not None:
            ...
            self.set(math.floor(self.func_time.take_step(self.value, t, delta_t, episode_delta_t)))
